...
function listeners() {
    ...
    btnGuardar.addEventListener('click', actualizarArticulo, false);
...
function irAPerfil(Email,Nombre) {
    setTimeout(function () {
        window.location.href = "Perfil.html?Nombre="+Nombre+'&Email='+Email;
    }, 0);
...
function loadNombreArticulo() {
    var Nombre = getParameterByName('Nombre');
    var outerHTML = '';
    outerHTML += Nombre;
    document.querySelector('#NombreArticulo').innerHTML = outerHTML;
}
...
function actualizarArticulo() {
    ...
    if (cNombre.length === 0 || cPrecioSalida.length === 0 || cCategoria.length === 0 || cCaja.length === 0) {
        ...
        deleteArticulo(getParameterByName('ID'));
        addNuevoArticulo();
        irAPerfil(getParameterByName('Email'), getParameterByName('Propietario'));
